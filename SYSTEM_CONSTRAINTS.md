# System Constraints & Physical Rules

ไฟล์นี้รวบรวมข้อจำกัดทางกายภาพและข้อกำหนดของระบบ เพื่อป้องกันการออกแบบ Workflow ที่ขัดแย้งกับความเป็นจริง

## 1. เครื่องนับรถ (Hardware Meters)
* **ข้อจำกัด:** เป็นเพียงจอตัวเลขโง่ๆ (Dumb Counter) ที่นับจำนวนเวลารถทับเซ็นเซอร์
* **ไม่มีออนไลน์:** ไม่มีการเชื่อมต่ออินเทอร์เน็ต ไม่มี Dashboard และ **ไม่สามารถออกรายงาน (Export Report) ได้**
* **การอ่านค่า:** เจ้าหน้าที่หรือ Audit ต้อง "เดินไปดูที่หน้าตู้" และจดด้วยมือ (Manual Record) เท่านั้น

## 2. ระบบ YC Parking
* **ลาน 2:** **มีระบบนี้** แต่ใช้สำหรับ "การสแกน QR Code บุคลากรพนักงาน" เท่านั้น (สามารถดึงรายงาน Dashboard ออนไลน์ได้)
* **ลาน 3:** **ไม่มีระบบนี้เลย** (ไม่มีบุคลากรจอดฟรี) การทำงานทั้งหมดเป็นเงินสดและพร้อมเพย์ลูกค้าทั่วไป

## 3. ช่องทางการชำระเงิน
* **อนุญาต:** เงินสด (Cash) และ สแกน QR PromptPay ของลูกค้าทั่วไป
* **ไม่อนุญาต:** **ไม่มีการรับเงินโอนเข้าบัญชี (Bank Transfer)** ในกระบวนการเก็บเงินรายวันของเจ้าหน้าที่ลานจอดรถเด็ดขาด เพื่อลดความซ้ำซ้อนในการตรวจสอบ

## 4. บัตรจอดรถฟรี (Free Parking Pass)
* **ลาน 2:** ยังคงมีกระบวนการรับบัตรจอดรถฟรี
* **ลาน 3:** ไม่มีนโยบายรับบัตรจอดรถฟรี

## 5. นโยบายการตรวจสอบของฝ่ายตรวจสอบภายใน (Internal Audit)
* **หลักการใหม่ (System Audit 100%):** เพื่อลดภาระงานของทีมออดิท ให้ Audit ปฏิบัติหน้าที่ในฐานะ Third Line of Defense โดยทำหน้าที่ "ตรวจสอบข้อมูลย้อนหลังจากระบบ (System Audit)" หรือทวนสอบจากรายงานของหน่วยงานอื่นเท่านั้น
* **การลงพื้นที่ (Physical Audit):** ห้ามไม่ให้ Audit เป็นผู้ลงพื้นที่สุ่มตรวจนับเงินสดหรือจดมิเตอร์หน้างานเป็นประจำ **ให้โอนภาระการลงพื้นที่สุ่มตรวจหน้างาน ไปเป็นความรับผิดชอบของ "หัวหน้างาน" หรือ "ฝ่ายการเงิน (ส่วนงานรับเงิน)"** แทน
* **การสอบทานของ Audit:** Audit จะทำหน้าที่ดึงข้อมูลใบสรุปยอด และรายงานการสุ่มตรวจหน้างานจากการเงิน มาเทียบเคียงกับระบบคาร์ปาร์ค เพื่อสอบทานว่ามีข้อผิดปกติหรือไม่ (Reconciliation)

## 6. การตั้งต้นยอดรถค้างจอด (Baseline Initialization)
* **ปัญหา:** การใช้สูตรคณิตศาสตร์ `(รถค้างจอด = ยอดยกมา + เข้า - ออก)` จะทำงานไม่ได้เลย หากไม่มียอดตั้งต้น (Baseline) ที่ถูกต้อง
* **การตั้งต้นครั้งแรก (The Big Count):** ต้องหาช่วงเวลาที่รถน้อยที่สุด (เช่น 03:00 น. ของวันหยุด) ปิดกั้นการเข้าออกชั่วคราว และ **"เดินนับจำนวนรถจริงด้วยสายตาทั้งลานเพียง 1 ครั้ง"** เพื่อเซ็ตเป็นยอดยกมาเริ่มต้น (Baseline)
* **การปรับฐาน (Baseline Audit):** มิเตอร์ฮาร์ดแวร์มีความคลาดเคลื่อนตามธรรมชาติ (เช่น มอเตอร์ไซค์ขับจี้ติดกัน หรือเบียดกันเข้า มิเตอร์นับ 1 แต่เข้า 2) ทำให้ยอดค้างจอดจากสูตรจะค่อยๆ เพี้ยน (Drift) เมื่อเวลาผ่านไป
* **ข้อกำหนด:** ต้องมีการทำ Physical Audit (เดินนับรถจริงทั้งลาน) อย่างน้อย **เดือนละ 1 ครั้ง** (เช่น คืนสิ้นเดือน ก่อนรีเซ็ตมิเตอร์ กค-2) เพื่อปรับปรุงตัวเลข "รถค้างจอด" ในสูตรให้ตรงกับความเป็นจริงเสมอ

## 7. การตรวจสอบยอดชำระ QR PromptPay (หน้างาน)
* **ลาน 2:** มี **"มือถือส่วนกลาง"** ประจำจุดจอด ซึ่งผูกกับแจ้งเตือนเงินเข้าธนาคารกรุงศรี เจ้าหน้าที่สามารถตรวจสอบยอดเงินเข้าได้ทันที
* **ลาน 3:** **ไม่มีมือถือส่วนกลาง** เจ้าหน้าที่ต้องใช้วิธี **"ถ่ายภาพสลิปของลูกค้า"** แล้วส่งเข้ากลุ่มไลน์ (LINE Group) ที่มีเจ้าหน้าที่การเงินอยู่ในกลุ่ม เพื่อเป็นหลักฐานยืนยันการรับชำระ
  * **⚠️ ความเสี่ยง (R6):** ลาน 3 ไม่สามารถตรวจสอบเงินเข้าได้แบบ Real-time ณ จุดจอด มีความเสี่ยงที่ลูกค้าอาจใช้ **สลิปปลอม หรือ สลิปเก่า** หลอกลวงเจ้าหน้าที่ได้

## 8. การเพิ่ม/แก้ไขจุดเสี่ยง (Risk Table Synchronization)
* **กฎเหล็กการแก้ไข HTML:** หากมีการเพิ่มหรือปรับปรุงจุดเสี่ยง (เช่น ใส่ `warn:'จุดเสี่ยง R6...'` ในส่วนของ JavaScript Data) **ห้ามลืม** เลื่อนลงไปเพิ่มข้อมูลจุดเสี่ยงนั้นในตาราง HTML `<table class="risk-table">` ด้านล่างสุดของไฟล์ด้วยเสมอ เพื่อป้องกันปัญหา Risk Code หลุดหรือข้อมูลไม่ตรงกันระหว่าง Process กับตารางสรุป
