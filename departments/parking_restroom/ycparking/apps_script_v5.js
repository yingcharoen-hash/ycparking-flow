function doPost(e) {
  try {
    ...
    let sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("บันทึกข้อมูล");
    sheet.appendRow([
...
function doGet(e) {
  if (e.parameter.action === "getInitialData") {
    ...
    let staffSheet = ss.getSheetByName("รายชื่อเจ้าหน้าที่");
  ...
  if (e.parameter.action === "getAllData") {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      // Determine format and extract fields robustly
      if (data[i].length > 20 && String(data[i][20]).includes("drive.google.com")) {
        ...
        isNewFormat = true;
        pdfUrl = data[i][19];
        photoSelfie = data[i][16] ? data[i][16].toString() : ""; // Fallback for shifted format
      ...
      // Final fallback: If we still don't have a selfie, but we see a Drive URL in column 16, 17 or 18, grab it
